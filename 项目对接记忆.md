...
**最新里程碑**:`(待推)` — v1.88 BUG-062 workflow_pipeline 路径漏防御(对齐旧路径)— 用户报"连续写作会丢东西"。根因:章节生成有两条路径(旧 `_handle_chapter_response` / 新 `workflow.on_ai_content`),旧路径有的 BUG-027 短回复哨兵 / pangu_meta 解析 / 伏笔同步 / 钩子爽点同步 / 13 法静扫 / 解析失败兜底,**新路径全漏**。结果:69 字 JSON 评分残留被当章节入库 → 落盘 → 下一章 prompt 把这串 JSON 当"上一章正文"喂给 AI → 雪崩污染。
**前几个里程碑**:
...
> **当前版本 `v1.88`**(每次推送后请同步更新此处)。**忘改 = 反例,跟"先推代码后推记忆"同等严重**。
> 详见"五十五、版本号铁律"那一节。
...
**最近更新**:2026-05-20 — 加给下个 Claude 警告条 11(PAT 撤销验证双重陷阱:`ls-remote` 对 public repo 无效 + 管道末端 $? 误判)
...
*最后更新:2026-05-20 / 加 Claude 常犯错警告 11(PAT 撤销验证双重陷阱)*
