...
**最新里程碑**:`(待推)` — v1.86 多视角反查("做没有的"系列后续延伸 3/4;右键剧情树节点 → 弹窗显示该节点关联的角色/伏笔/承诺/关系/信息;纯查询 + UI 弹窗,不动数据/不调 AI/不加 pipeline;复用 v1.85 chapter_links + v1.80 ch_range 算出"该节点关联章号集合"chs,扫各库找章号 ∈ chs 条目)
**前几个里程碑**:
...
> **当前版本 `v1.86`**(每次推送后请同步更新此处)。**忘改 = 反例,跟"先推代码后推记忆"同等严重**。
> 详见"五十五、版本号铁律"那一节。
...
**最近更新**:2026-05-19 — v1.86 多视角反查(右键剧情树节点 → 反查关联 6 库;纯查询不调 AI 不动数据)
...
*最后更新:2026-05-19 / v1.86 多视角反查(右键剧情树节点反查 6 库)*
