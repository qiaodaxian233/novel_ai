...
**最新里程碑**:`(待推)` — v1.80 BUG-060 剧情树规划("做没有的"第 4/4 步 = **全系列完成**;QTreeWidget 4 层 / 节点 ID remap + 悬挂处理 / 注入按章号定位最具体节点;**首次引入树形 UI** 与 6 库表格差异化)
**前几个里程碑**:
- `7395873` — v1.79 BUG-059 信息隔离控制(2 表外键引用 + 侦测违规模式)
- `2814df7` — v1.78 BUG-058 剧情进度管理(3 子表:弧线/关系值/目标;delta 累加 + 双 clamp)
- `8adb9f9` — v1.77 BUG-057 威胁承诺自动闭环(复刻 v1.76 伏笔模式)
- `f29b3ca` — v1.76 BUG-056 全自动伏笔闭环
...
**最近更新**:2026-05-19 — v1.80 BUG-060 剧情树规划("做没有的"第 4/4 步 = **全系列结束**;首次 QTreeWidget;注入按章号定位最具体节点)
...
*最后更新:2026-05-19 / v1.80 BUG-060 剧情树规划("做没有的"第 4/4 步 = 全系列结束)*
