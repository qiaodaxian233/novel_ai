...
**最新里程碑**:`(待推)` — v1.79 BUG-059 信息隔离控制("做没有的"第 3/4 步;2 表 via info_id 外键:tbl_infos + tbl_known_by;info_check 是【侦测穿帮】不自动修;自动续号 INFO-001 + id_remap + 悬挂引用过滤;pipeline 6 阶段紧链)
**前几个里程碑**:
- `2814df7` — v1.78 BUG-058 剧情进度管理(3 子表:弧线%/关系值矩阵/目标;delta 累加 + 双 clamp)
- `8adb9f9` — v1.77 BUG-057 威胁承诺自动闭环("做没有的"第 1/4 步;复刻 v1.76 伏笔模式)
- `f29b3ca` — v1.76 BUG-056 全自动伏笔闭环(消伪超期 + AI 自动检查回收 + 强约束注入)
- `920286a` — v1.75 BUG-055 Canon 自动抽取可见性 + 全链路诊断
- `52cafc7` — v1.74 BUG-054 战力体系自动抽取 + hero_state 快照模式根因修复
- `40f8152` — v1.73 fix BUG-053 章末钩子频繁误判触发死磕
...
**最近更新**:2026-05-19 — v1.79 BUG-059 信息隔离控制("做没有的"第 3/4 步;2 表外键引用 + 侦测违规模式;v1.80=剧情树 待做)
...
*最后更新:2026-05-19 / v1.79 BUG-059 信息隔离控制("做没有的"第 3/4 步)*
