...
*最后更新:2026-05-22 / **v2.00 模块化拆分 P1**:`core/` 子包诞生 —— `constants.py`(105 行,9 个 UI/数据常量)+ `stylesheet.py`(54 行,QSS 全局样式)。`novel_ai.py` 20740→20595 行(-145),APP_VERSION v1.98→**v2.00**。顺手做掉 BUG-072 留下的硬钉 TODO(test_bug071/072 的 TestAppVersion 改 `re.search + ≥` 模式)。424 测试零退化。下一步 P2:抽 PROMPTS 字典(583 行,29 keys)到 `core/prompts.py`。前轮:`8142921` 记忆校准修订;再前轮:v1.97 BUG-071 字典治本。*
