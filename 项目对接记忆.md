...
*最后更新:2026-05-21 / **记忆校准修订**:由 Claude 跑全仓静态扫描(`git clone` + `wc -l` + `grep`)对齐 v1.98 真实状态 —— `novel_ai.py` 行数 8233→**20732**;PROMPTS keys 17→**29** + 行号 93-333→**140-723**;集成激活代码行号 334-342→**725-729**;文件结构 8 .py→**18 .py**;测试 49 个文件;新增 ⓪⓪.5 GitHub Web UI 截断陷阱警告。本次只动文档不动代码,不升 APP_VERSION。前轮:v1.97 BUG-071 字典治本。*
