...
**最新里程碑**:`(待推)` — **v2.02 🎯 模块化拆分 P3 — `ui/` 子包 + `core/site_profiles.py`**(5 个完全自包含的 UI 类 + 1 个 DOM 选择器档案外迁)。新建 `ui/` 子包,抽出 5 个类:`_PanguForbiddenHighlighter`(50 行,章节编辑器禁用词高亮)→ `ui/highlighters.py`;`_TTSSynthThread`(33 行,v1.10 TTS 后台线程)→ `ui/threads.py`;`ThemeManager`(224 行,亮暗主题切换)→ `ui/theme.py`;`ConversationSwitcher`(167 行,E 模块对话槽管理器)→ `ui/conversation_switcher.py`;`StoryOutline`(116 行,故事大纲 Tab)→ `ui/story_outline.py`。同时把 `SITE_PROFILES`(131 行,8 站点 DOM 档案)→ `core/site_profiles.py`。**主程序 20015→19295 行,瘦 720 行**(累计 P1+P2+P3:20740→19295,共瘦 1445 行 / 7.0%)。APP_VERSION v2.01→**v2.02**。**预先依赖分析**:用 AST 扫了每个类对项目内全局符号的引用,5 个类全部完全独立,零项目内依赖(`ThemeManager` 唯一命中的 "MainWindow" 是 QSS 字符串里的 `QMainWindow` 子串,假阳性)。**采用从下往上抽取避免行号偏移**:`ConversationSwitcher`(10106-10272)→ `SITE_PROFILES`(7485-7615)→ `StoryOutline`(2760-2875)→ `_TTSSynthThread`(442-474)→ `ThemeManager`(216-439)→ `_PanguForbiddenHighlighter`(166-215),Python AST 一次性抽取脚本批处理。**测试零退化**:424 passed(BUG 136 + workflow 31 + research 123 + lifespan 68 + lifespan_panel 66),与 P1/P2 基线完全一致。**给下一代 Claude 的接力告示**:P1+P2+P3 已收官,主程序剩 19295 行,主要还是 `MainWindow`(~9400 行)+ `CharacterLibrary`(3862 行)+ `BrowserWorker`(2480 行)+ `CreationSettings`(1393 行)三大块。**下一步 P4**:5 个中型 Tab(`ChapterEditor` 409 行 / `SkillLibrary` 388 行 / `GenerationControl` 348 行 / `ProjectHomeTab` 309 行 / `DialogMemory` 280 行 / `CanonGuard` 215 行 / `BookSplitterTab` 172 行 / `CreationSettings` 1393 行),总约 3300 行。每个 Tab 都是 QWidget 子类,需要先做依赖分析(部分可能引用 MainWindow 或其他类)。**`ui/__init__.py` 已标好** P4 起会建 `ui/tabs/` 子目录放 Tab 类。

**前一个里程碑**:`cd4cc3f` — v2.01 P2 PROMPTS 字典外迁。
**再前**:`fd1b0eb` — v2.00 P1 core/ 子包(constants + stylesheet)。
**再再前**:`8142921` — v1.98 记忆校准修订(纯文档)。
