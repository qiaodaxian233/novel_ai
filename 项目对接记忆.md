...
**最近更新**:2026-05-21 — **v1.97 🎯 BUG-071 `_pending_task_target` 改字典治本(B-2,B 完整收官)**。`_pending_task_target` (单变量) → `_pending_task_targets` (字典 key=label/task_id);14 处真实读写点全部迁移(init + 主写 + 主路径 12 处读/pop + 死磕 + 手动抓取 + dead code 2 处)。旧字段保留 `= None` 做兼容兜底,任何外部代码若残留访问拿 None 不崩。21 BUG-071 测试 + 既有 110 全过 = **131/131**。**严重失误反省**:上一轮 Claude 做完 v1.97 改动 + 本地 commit 后,**在 push 前**就执行了 `rm -rf push_work` — 本地 commit 永久丢失。**铁律补充**:**清场必须严格在远端二次确认 commit hash 之后**,不能在 commit-after-push 的同一批清理动作里混做。`本轮重做。前次:2026-05-21 — v1.96 🛡 BUG-070 单章模式"批量已结束"误报防御(治标,留治本);v1.95 📏 BUG-069 字数判定三档(A→B→C→D 路线第一步)。
...
*最后更新:2026-05-22 / **v2.02 模块化拆分 P3**:`ui/` 子包诞生 + `core/site_profiles.py`。5 个完全自包含的 UI 类抽出(`_PanguForbiddenHighlighter` / `_TTSSynthThread` / `ThemeManager` / `ConversationSwitcher` / `StoryOutline`,共 590 行)+ SITE_PROFILES 131 行 DOM 档案。`novel_ai.py` 20015→19295 行(-720),累计 P1+P2+P3 共瘦 1445 行 / 7.0%。APP_VERSION v2.01→**v2.02**。预先 AST 扫依赖确认零项目内引用,从下往上抽取避免行号偏移。424 测试零退化。下一步 P4:5 个中型 Tab(约 3300 行)。前轮:`cd4cc3f` v2.01 P2 PROMPTS 外迁;再前:`fd1b0eb` v2.00 P1 core/ 子包。*
