...
**最新里程碑**:`(待推)` — **v2.05 🎯 模块化拆分 P6 — `ui/browser_worker.py`(`BrowserWorker` 2475 行外迁,主程序破万行!)**。把 33 方法 / 2475 行的 `BrowserWorker` 类整体抽到 `ui/browser_worker.py`(127 KB,P6 阶段单文件)。**主程序 12125→9650 行,瘦 2475 行**(**主程序破万行!累计 P1+P2+P3+P4+P5+P6:20740→9650,共瘦 11090 行 / 53.5%**)。APP_VERSION v2.04→**v2.05**。**预先 AST 依赖扫描确认完全自包含**:33 方法**零项目内类依赖**(不引用 MainWindow / CharacterLibrary / 任何 Tab),只依赖 PyQt5(QObject + pyqtSignal)+ Selenium(webdriver / ActionChains / By / Keys / ChromeOptions / EdgeOptions / WebDriverException)+ stdlib(`os` `sys` `time` `threading` `json` `queue` `glob` `socket` `subprocess` `tempfile` `traceback` `Path` `urlparse`)。**关键设计**:`SELENIUM_AVAILABLE` 走 P4 同模式,`ui/browser_worker.py` 顶部自己 try/except,主程序保留不变。**抽取方法**:一次性 AST 脚本(`/home/claude/extract_p6.py`),定位 L241-L2715,提取后写带 header 的新文件,主程序原段删除。**踩坑:CRITICAL_TARGETS 误判**:AST 脚本初版把方法内的局部变量 `CRITICAL_TARGETS` 误当成外部依赖,生成了 `from core.constants import CRITICAL_TARGETS` 这条不存在的 import 导致 ImportError。修复:删掉错误 import,验证它确实是方法内局部赋值(`_send_prompt` 第 973 行 `CRITICAL_TARGETS = {...}`)。**教训**:AST 扫"外部引用"需要排除方法体内的 Assign 节点。**测试静态扫描适配**:14 个测试挂(test_bug065 全套,因为 `_build_degraded_content` / `_send_prompt` / `_get_send_button_state` / `_dispatch_send` 都迁了)。修法:test_bug065 顶部加 `BROWSER_WORKER_PATH = HERE/ui/browser_worker.py` + `_load_all_sources()` 工具函数(为后续 P 阶段做铺垫);改 4 处独立 with-open 从读 `NOVEL_AI_PATH` 改成读 `BROWSER_WORKER_PATH`;`extract_method_source` 调用也从主程序改成 BrowserWorker 模块。**测试零退化**:424 passed,完全对齐 P1~P5 基线。**给下一代 Claude 的接力告示**:P1~P6 已收官,主程序剩 **9650 行**(从 20740 行瘦 53.5%,**单文件首次破万**)。剩下的最大头是 `MainWindow`(~9400 行,装配中心)— **强烈建议不拆**:它是 entry point,11000+ 行方法相互调用密度极高,拆 mixin 收益小、易引入 bug、增加调试复杂度。**P6 经验教训**:(1)AST 扫"外部引用"必须排除方法体内的 `ast.Assign` 节点,否则会把方法内局部变量误当外部依赖;(2)Selenium 子模块按需局部 import 是个聪明的现有设计(13 处 `from selenium.X import Y` 散在方法体内),P6 抽走后不需要全提到顶部,保留原貌即可;(3)P6 是模块化拆分的"自然完结点"—— `core/` `ui/` `ui/tabs/` 三层结构清晰,`MainWindow` 留在主文件作为装配中心是合理的架构。
