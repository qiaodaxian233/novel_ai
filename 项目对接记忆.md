...
**最新里程碑**:`(待推)` — v1.89 BUG-028 回归修复(指纹防串安装位置错位)— 用户实测 v1.88 后报"怎么还给以前的 BUG 整出来了"。根因不是回归,是 BUG-028 当年修的时候**位置就放错了**:`prev_response_fingerprint` 应在 `_inject_prompt` 之前捕获,实际却在 `_dispatch_send` 之后捕获(注释和代码语义偏移)。DeepSeek 收到 prompt 瞬间会在 DOM 插空 assistant 占位槽,导致指纹 = `"|0"`,后续抓到章节残留(2960 字)永远不等于 `"|0"`,防串失效。修法:把指纹块挪到 line 9102(发送前真正的"发送前"区段)。潜伏 5 天,因单元测试 mock 不出 DOM 时序而未暴露。
**前几个里程碑**:
...
> **当前版本 `v1.89`**(每次推送后请同步更新此处)。**忘改 = 反例,跟"先推代码后推记忆"同等严重**。
> 详见"五十五、版本号铁律"那一节。
...
**最近更新**:2026-05-20 — v1.89 BUG-028 回归(指纹捕获位置错位,从 _dispatch_send 后挪到 _inject_prompt 前)
...
*最后更新:2026-05-20 / v1.89 BUG-028 回归(指纹捕获位置错位修复)*
