...
**最新里程碑**:`(待推)` — v1.93 👤 BUG-067 角色 `last_ch` 字段 + 🔍 同名不同姓检查(借鉴竞品"由风写作 v1.1.7"长篇人物管理痛点,跟 v1.92 BUG-066 章节锁定是**独立改动 — v1.92 动章节,v1.93 动角色,两者完全不重叠**)。**起因**:v1.92 推完后用户说"继续",直接进 v1.93。Claude 调研发现 `first_ch`(首次出场)字段**已经存在**于 prompts(L282 world_extract 模板)和 UI 表格(列 7),只缺 `last_ch`(退场章节)— 工程量比预想小。**功能 1:同名检查**:新方法 `_find_duplicate_names(rows_data)`(staticmethod,纯函数,无 Qt 依赖,便于单测)+ UI handler `_on_check_duplicate_names`(读 tbl_chars → 调纯函数 → 弹窗显示冲突)+ 顶部按钮「🔍 同名检查」。只做完全重名检测(包含/被包含留 v1.94)。**功能 2:`last_ch` 字段**:tbl_chars 列数 8→9,放在 first_ch 之后(列 8,新最后列);AI prompts **不加** `last_ch`(AI 无法预知未来,留用户手动标);加载兜底 `c.get("last_ch", "")` 兼容旧存档/新生成数据。**关键漏改修复**:除了 `_build_characters_tab` 表格定义 / `_add_character` defaults / `serialize` 路径 / `DICT_KEY_MAPS` / `DICT_KEY_MAPS_LOCAL` / load vals 这 6 个明显点,**还有 2 个隐藏的 8 列硬编码**:(a) `_refresh_relation_graph` L6028 `_tbl_to_rows(self.tbl_chars, 8)` — **故意保持 8**,因为 relation_graph.build_graph_data 接口按 8 列设计,加 9 也会被它在 row[0:8] 截掉,加注释明示;(b) `load` 路径 L6158 `list_to_tbl(self.tbl_chars, ..., 8)` — **必须改 9**,否则 normalize 转出的 9 元素 list 喂进来时第 9 列(last_ch)永远填不进表(沉默 bug)。测试加专门一条 `test_load_path_uses_9_columns` 防回归。**另两个"故意 8"的语义点**:L6755 / L15791 在 prompt 注入路径里 `for c in range(8)` + `name, role, ..., _ = row` unpack,只取前 8 列(本来就不用 first_ch,更不用 last_ch),保持 8 不动 — 这里如果未来要给 AI 提示"某角色已退场",改成 9 + 处理 last_ch 即可。**测试 24 条全过**:8 条数据模型(DICT_KEY_MAPS / round-trip / 加载兜底 / vals 顺序)+ 9 条同名检查纯函数(正例/反例/边界/空表/单角色/大小写)+ 5 条 UI 源码断言(列数 9 / header 顺序 / defaults 长度 / 按钮存在 / serialize 9)+ 2 条 v1.92 不被破坏(章节锁定方法仍在 / locked 默认值未动)。**合跑回归 75 条**(24 BUG-067 + 15 BUG-066 + 17 BUG-065 + 19 housekeeper)全过。本次仅触动 `novel_ai.py`(9 处)+ 新增 1 个测试文件,**完全不动 v1.92 改过的章节相关代码**(隔离性纪律)。
**前几个里程碑**:
...
**最近更新**:2026-05-21 — v1.93 👤 BUG-067 角色 last_ch 字段 + 同名不同姓检查上线。借鉴竞品由风写作 v1.1.7 长篇人物管理痛点。tbl_chars 列 8→9,加退场章节列;新方法 _find_duplicate_names(纯函数,可单测)+ UI handler + 顶部"🔍 同名检查"按钮。**关键漏改修复 L6158 load 路径列数(沉默 bug)**,加注释明示 L6028 _refresh_relation_graph 故意保持 8 列(relation_graph 接口设计如此)。新测试 24 条全过,合跑 75 条全过。本次仅触动 novel_ai.py(9 处),完全不动 v1.92 章节锁定代码(隔离性纪律)。
...
*最后更新:2026-05-21 / v1.93 BUG-067 角色 last_ch 字段 + 同名检查(竞品分轮第二轮)*
