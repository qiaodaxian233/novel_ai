...
**最新里程碑**:`(待推)` — v1.96 🛡 BUG-070 单章模式"批量已结束"误报防御 + 串台诊断(A→B→C→D 路线第二步,**治标 + 留治本**)。**起因**:v1.91 时实战日志暴露"批量生成已结束"在单章模式触发,v1.91 当时只修了下游(摘要丢失)没动这条 log。v1.95 实战日志(15:26:02)又出现 — 用户单章模式跑,但 "批量已结束" 仍触发,**且时序怪异**:Canon 抽取响应回来时立刻出"✓ 摘要已记入对话记忆"+"已自动保存"(明明是 chapter_summary handler 的 log),然后才"批量已结束",**最后摘要任务才真发**。**根因调研**:`_pending_task_target` 是**单变量**(L12336),`_send_to_ai` 时覆盖式赋值。Worker 是串行 FIFO 队列,但 main UI 端连续提交两个 _send_to_ai(Canon 抽取 + 摘要)→ `_pending_task_target` 只保留**最后一个**的 meta。第一个响应回来时 `_on_response_received` 读 `_pending_task_target` → 拿到**错误的 meta** → 路由错乱 → 摘要 handler 处理 Canon 抽取的 1174 字内容 → 把 Canon 内容当摘要写到 `ch["summary"]`!**治本(留 v1.97 B-2 单独做)**:`_pending_task_target` 改 dict 映射 `task_id → meta` — 涉及 8+ 处读写,跟 pangu/laodao 已存在的 task target 路由互动复杂,工程量大,单独一轮才稳。**本轮治标**:`chapter_summary` handler 末尾的 elif 链改三档守卫 — (1) `chain_to_next AND _batch_remaining > 0 AND not _batch_paused` → 触发下一章(真批量正常推进);(2) `chain_to_next AND (_batch_remaining > 0 OR _batch_paused)` → "批量已结束"(真批量被暂停 / 边界);(3) `chain_to_next AND _batch_remaining == 0 AND not _batch_paused` → **打 warn 不打"批量已结束"**(单章模式 chain_to_next 必然是串台,留诊断 log 给下次实战定位)。**测试 9 条全过**:3 条源码守卫断言(BUG-070 标记 / 三档结构 / 无裸 elif)+ 1 条 end_batch step 独立性(确认 L17922 不受影响)+ 5 条 v1.92-v1.95 完整性。**合跑 110 条全过**(9 BUG-070 + 19 BUG-069 + 7 BUG-068 + 24 BUG-067 + 15 BUG-066 + 17 BUG-065 + 19 housekeeper)。本次仅触动 `novel_ai.py`(2 处:APP_VERSION + chapter_summary handler 末尾)+ 新增 1 测试。**给下一代 Claude 的诚实告示**:这只是治标,根因 race 还在;下次实战如果出 `⚠ chain_to_next=True 但 _batch_remaining=0` 这条 warn,就是 race 又触发了 — 那时候必须做 B-2 治本(改字典)。**A→B→C→D 进度**:A ✅ v1.95 字数三档 → B ✅ v1.96 批量已结束防御(治标)→ C(区间审查 + 逐级清除)→ D(让 AI 用 last_ch)。
**前几个里程碑**:
...
**最近更新**:2026-05-21 — v1.95 📏 BUG-069 字数判定三档(A→B→C→D 路线第一步)。新增 `word_count_long` 字段(不动 `word_count_ok` 语义,既有 19 测试一字未改全过);`record_word_count` 加上限 1.5x;oneliner 三档 emoji `✓/⚠/✗`;`_compute_health` 不动(超长 ⚠ 不扣健康度)。实战数字 3088/2300=1.34x 从 ✗ 变 ✓,健康度从 🟡73% 自动回升到 🟢 区。新测试 19 条全过,合跑 101 条全过。本次只动 housekeeper.py(3 处)+ novel_ai.py(1 处版本号)+ 1 测试。下一轮 B:批量已结束治本。
...
*最后更新:2026-05-21 / v1.96 BUG-070 单章模式"批量已结束"误报防御 + 串台诊断(A→B→C→D 第二步,治标+留治本)*
