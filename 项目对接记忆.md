...
**最新里程碑**:`(待推)` — **v2.01 🎯 模块化拆分 P2 — `core/prompts.py`(PROMPTS 字典外迁)**。从 `novel_ai.py` 第 148-731 行抽出 PROMPTS 字典(29 keys,584 行)到 `core/prompts.py`,内容**零修改原文搬运**;顶部加 `from core.prompts import PROMPTS`;**主程序 20595→20012 行,瘦 583 行**(累计 P1+P2:20740→20012,共瘦 728 行)。APP_VERSION v2.00→**v2.01**(P 系列中末位 +1)。**关键风险点验证通过**:`pangu_patch.install_pangu(globals())` 通过 `globals()['PROMPTS']` 拿到的字典引用,与 `core.prompts.PROMPTS` 是同一对象(Python import 机制),`PROMPTS[key] = head + original + tail` 的 in-place 修改对两边都生效。沙箱实测:`chapter` 键从 475 字符被 pangu 加到 4932 字符,patch 后跨 import 仍拿到改过的版本,`P_again is P_orig == True`。**core/prompts.py 头部加 18 行兼容性 docstring**,显式警告:"如果未来要把 PROMPTS 改成只读 / MappingProxyType / 冻结模式,必须同步改 pangu_patch.py 走'复制再替换'路径,否则 patch 静默失效"——这是给下一代 Claude 的雷区标记。**测试零退化**:BUG-065~072 套件 136 passed + workflow_panel 31/31 + research_report_skills 123/123 + lifespan_loops 68/68 + lifespan_loops_panel 66/66,**合计 424 passed**,与 P1 基线完全一致。**下一步 P3**:`site_profiles.py`(各 AI 网站 DOM 选择器,约 200 行)+ 5 个小独立类(`_PanguForbiddenHighlighter` 50 行 / `_TTSSynthThread` 35 行 / `ThemeManager` 226 行 / `ConversationSwitcher` 169 行 / `StoryOutline` 121 行),总约 800 行,风险仍低(都是基本自包含的类)。

**前一个里程碑**:`fd1b0eb` — v2.00 P1 `core/` 子包(constants + stylesheet)。
**再前**:`8142921` — v1.98 记忆校准修订(纯文档)。
...
*最后更新:2026-05-22 / **v2.01 模块化拆分 P2**:PROMPTS 字典(584 行 / 29 keys)外迁到 `core/prompts.py`。`novel_ai.py` 20595→20012 行(-583),累计 P1+P2 共瘦 728 行(20740→20012)。APP_VERSION v2.00→**v2.01**。**关键风险点验证**:pangu_patch.install_pangu 的就地修改机制完全保留(沙箱实测 chapter 键 475→4932 字符,跨 import 同对象引用)。core/prompts.py 头部加显式雷区警告:未来如果改 PROMPTS 为只读必须同步改 pangu_patch。424 测试零退化。下一步 P3:site_profiles + 5 个小独立类。前轮:`fd1b0eb` v2.00 P1 core/ 子包。再前轮:`8142921` 记忆校准修订;再再前轮:v1.97 BUG-071 字典治本。*
