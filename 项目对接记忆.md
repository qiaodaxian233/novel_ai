...
**最新里程碑**:`(待推)` — v1.94 🎨 BUG-068 下一章选项按钮对比度修复(UI 可读性,**1 行 CSS 改动**)。**起因**:用户截图反馈"下面按钮看不清" — 章节末"📌 本章元信息"面板里的三个"下一章选项"按钮文字几乎不可见。**根因**:`ChapterEditor._set_pangu_meta_display` 行 2062-2065 的 `btn.setStyleSheet` **漏写 `color` 字段**,文字用 Qt 默认前景色;米色背景 `#fff8ea` + 默认浅灰文字 = 对比度极低。Panel 内其他 label(钩子/爽点/伏笔/tip)全部显式 `color:#444`/`#666` 等 — 说明 panel 设计语义本来就要求显式 color,**按钮漏写是真 bug,不是风格选择**。**修法**:正常态加 `color:#3a2a10`(深棕,配米色背景对比度 >7:1,WCAG AAA),hover 态 `color:#000`(纯黑给明确点击反馈)。**测试**:`test_bug068_next_option_btn_contrast.py` 7 条全过 — 4 条 UI 源码断言(setStyleSheet 含 color / 用深棕色 / hover 含 color / panel 其他 label 早就有 color 这个对照证据)+ 2 条 v1.92/v1.93 不被破坏(章节锁定 + 角色 last_ch 字段仍在)+ 1 条版本号 ≥ v1.94。**合跑回归 82 条全过**(7 BUG-068 + 24 BUG-067 + 15 BUG-066 + 17 BUG-065 + 19 housekeeper)。本次仅触动 `novel_ai.py`(2 处:APP_VERSION + 1 处 setStyleSheet)+ 新增 1 测试。
**前几个里程碑**:
- `ce23313` — v1.93 BUG-067 角色 last_ch 字段 + 同名不同姓检查。tbl_chars 列 8→9 + `_find_duplicate_names` staticmethod 纯函数 + UI handler。**关键漏改修复 L6158**(load 路径列数沉默 bug),加注释明示 L6028 / L6755 / L15791 "故意 8" 的语义。完全不动 v1.92。
- `9b937a8` — v1.92 BUG-066 章节锁定机制 MVP(借鉴竞品中稿/终稿,跟 v1.91 同源焦虑)。章节 dict 加 locked 字段 + 4 个改动入口拦截 + 右键菜单锁/解锁 + 🔒 前缀显示。命名避让原有 toggle_lock(编辑器只读切换)。
- `10d5839` — v1.91 BUG-065 关键后处理任务发送失败 → 重试 + 本地降级(摘要丢失止血)。用户点醒"检测发送键状态"切换"事后症状判定 → 按钮态判定";`_get_send_button_state` 五态机 + `_dispatch_send` 4 检测信号 + `_send_prompt` 关键任务分级 + `_build_degraded_content` 头 300 尾 300 降级。
- `c16db8b` — v1.90 推送后遗漏的"最新里程碑"标签同步(纯 docs)
- `a7d2b61` — v1.90 📋 管家 P1(章末元监督上线)
- `a36bbeb` — v1.89 BUG-028 回归修复(指纹防串安装位置错位,从 _dispatch_send 后挪到 _inject_prompt 前)
**前几个里程碑**:
...
**最近更新**:2026-05-21 — v1.94 🎨 BUG-068 下一章选项按钮对比度修复。setStyleSheet 漏写 color → 米色背景上文字几乎不可见。修法:加 `color:#3a2a10`(深棕,对比度 13:1)+ hover `color:#000`。值得记录的不是这 1 行 CSS,而是"漏写 setStyleSheet 字段 = 用默认值,默认值不一定如你所想"这条 Qt 陷阱 + "UI bug 必须靠截图暴露"这条实战纪律。新测试 7 条全过,合跑 82 条全过。本次仅触动 novel_ai.py(2 处)+ 1 测试。
...
*最后更新:2026-05-21 / v1.94 BUG-068 下一章选项按钮对比度(Qt setStyleSheet 漏写陷阱 + UI bug 必须靠截图暴露)*
