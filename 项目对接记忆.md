...
**最新里程碑**:`(待推)` — v1.83 用户可见 dialog/log 文案补扫(v1.82 漏了 7 处:dialog 标题/log 启动行/log 解析失败/log 全空/[来源]标签/同步失败/关于框 HTML;剩余 17 处全是 docstring/注释)
**前几个里程碑**:
- `8698164` — v1.82 文档卫生 + UI 文案修正(顶层 docstring / checkbox / 按钮 / tooltip 等 8 处)
- `ede0558` — v1.81 BUG-061 评分门校准 + 死磕精确定位(95 分门一次过)
- `9f146e7` — v1.80 BUG-060 剧情树规划("做没有的"4/4 系列完成)
- `7395873` — v1.79 BUG-059 信息隔离控制
- `2814df7` — v1.78 BUG-058 剧情进度管理
...
> **当前版本 `v1.83`**(每次推送后请同步更新此处)。**忘改 = 反例,跟"先推代码后推记忆"同等严重**。
> 详见"五十五、版本号铁律"那一节。
...
| 🟡 中 | **角色 POV 模式**(配合 v1.79 信息隔离 — 只让 AI 看到该 POV 角色已知的信息生成) | 3-4 h | ⏳ 用户提需求时再做。这条性价比最高,因为直接复用 v1.79 已建好的 info_id + known_by 数据结构 |
| 🟡 中 | **跨表关联可视化**(剧情树节点 ↔ 伏笔 ↔ 承诺 ↔ 关系值 用图谱串起来) | 6-8 h | ⏳ 用户提需求时再做。需要新建 QWebEngineView + cytoscape.js 之类的图谱库 |
| 🟡 中 | **多视角反查**(从某节点反查"用到该节点的所有伏笔/承诺/角色") | 5-7 h | ⏳ 用户提需求时再做 |
| 🟡 中 | **写作模式回流**(章节生成时 reverse 引用回剧情树定位) | 4-6 h | ⏳ 用户提需求时再做 |
...
**最近更新**:2026-05-19 — v1.83 UI 文案补扫(v1.82 漏的 7 处 dialog/log/关于框 + 一处 "5 类" 过时数据修正)
...
*最后更新:2026-05-19 / v1.83 UI 文案补扫(v1.82 漏网)*
