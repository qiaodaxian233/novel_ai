...
**最新里程碑**:`(待推)` — **v2.06 🐛 BUG-073 + BUG-074 双 bug 修复(模块化拆分留下的隐藏地雷)**。

**BUG-073** 起因:P1 拆分时沙箱跑 `test_v6.py` 暴露,但当时记为"pre-existing,跟 P1 无关",留到 v2.06 才修。**根因**:`ui/tabs/creation_settings.py` 的 `load_settings()` 方法 4 处 `s.value("xxx", [])` 在 Linux PyQt5 + offscreen 无存档时返回 None,后面 `for n, cb in ...: cb.setChecked(n in gfs)` 抛 TypeError。Windows 因 QSettings 序列化方式差异不触发。**修法**:4 处都加 `or []` 兜底(`s.value("xxx", []) or []`),最小改动,不影响 Windows 行为。覆盖 `genres` / `endings` / `golden_fingers` / `personas` 四个 list 类设置。

**BUG-074** 是修 BUG-073 时**实战验证发现的连锁 bug**:跑 `mw.tab_settings.load_settings()` 验证修复时,触发 `ui/story_outline.py:35` `Qt.AlignTop` NameError。诊断后发现 P3 模块化拆分时**漏了一批 PyQt5 import**:
  - `ui/story_outline.py`:漏 `Qt` + `QSpinBox`
  - `ui/conversation_switcher.py`:漏 `QCheckBox` / `QGroupBox` / `QColor`

之前没暴露是因为这些代码路径在测试里没被走到(GUI 实际 show 才触发),`__init__` 阶段不执行 `Qt.AlignTop` 这种 attribute 访问。**修法**:补全 import。**写了静态扫描工具**(`/home/claude/scan_pyqt_imports.py` + `test_bug074_ui_imports.py::test_all_ui_modules_clean`)— 用 AST 收集所有 PyQt5 类引用 vs import 列表,差集就是漏 import,扫了 P3~P6 所有拆分文件确认其余全干净。

**关键教训(给下一代 Claude)**:**P3~P6 的 AST 抽取脚本只复制了类源码 + 顶部基础 import,没补全所有引用 → 沉默 bug**。修法 1:每次拆分后**必须跑一遍 `test_bug074_ui_imports.py` 巡检** + 实际 `QApplication([]) + MainWindow() + 各 Tab 关键方法调用` smoke test。修法 2(治本):未来如果还要拆,AST 脚本要扩展成"扫所有 PyQt5 Name/Attribute 引用并自动补 import"。

**改动汇总**:
  - `ui/tabs/creation_settings.py`:4 处 `or []` 兜底(BUG-073)
  - `ui/story_outline.py`:补 `Qt` + `QSpinBox` import(BUG-074)
  - `ui/conversation_switcher.py`:补 `QCheckBox` / `QGroupBox` / `QColor` import(BUG-074)
  - `novel_ai.py`:APP_VERSION v2.05 → **v2.06**
  - 新增 `test_bug073_qsettings_none_list.py`(6 测试)
  - 新增 `test_bug074_ui_imports.py`(3 测试,含全 ui/ 模块巡检)

**测试零退化**:**433 passed**(145 bug-065~074 + 288 v2 套件)。

**前一个里程碑**:`818d814` — v2.05 P6 ui/browser_worker.py(BrowserWorker 2475 行外迁,主程序破万行!累计 P1~P6 共瘦 11090 行 / 53.5%)。
**再前**:`c03737f` — v2.04 P5 ui/tabs/character_library.py(CharacterLibrary 3860 行外迁,瘦 3857 行)。
