...
**最新里程碑**:`(待推)` — v1.91 🛎 BUG-065 关键后处理任务的发送失败重试 + 本地降级兜底。v1.90 实战暴露:摘要任务发送失败 → `_dispatch_send` "事后症状全阴性"(消息数不变 / textarea 未清空 / 无 AI 写迹象),直接放弃,**章节摘要丢失,下一章看不见本章**。用户点醒 Claude 的关键问题:"**检测发送键处于什么状态这个没用吗?**" — 让 Claude 从"事后症状判定"切换到"按钮当前态判定"。新增 `_get_send_button_state()` 五态状态机(enabled/disabled/stop/loading/none)、`_dispatch_send` Enter 后第 4 检测信号(按钮 stop/loading = AI 已接管)、`_send_prompt` 失败路径分级处理(关键任务 ∈ {chapter_summary, canon_extract, character_extract, world_extract, long_term_extract} → 重试 2 次 + 本地降级,普通任务维持原放弃语义)、`_build_degraded_content` 章节摘要降级(头 300 + 尾 300 + [降级 BUG-065] 标签)、`_send_to_ai` 透传 `_xxx` 前缀 extra 字段到 worker(否则 worker 端拿不到 `_ch_content`,降级退化为占位串)。**编号插曲**:Claude 先用 BUG-063 立案,grep 发现 v1.86 多视角反查已占 BUG-063、v1.87 跨表关联已占 BUG-064 → 改用 BUG-065。测试 17 条全过 + housekeeper 19 条回归全过。本次仅触动 `novel_ai.py`(5 处)和新增 1 个测试文件。
**前几个里程碑**:
...
**最近更新**:2026-05-20 — v1.90 📋 管家 P1 上线(章末元监督)。新文件 housekeeper.py 380 行 + test_housekeeper.py 256 行,主路径 _accept_chapter_and_continue 和 workflow 路径 _accept 都挂了 hook,工具菜单加管家日报入口。29 测试全过。
...
*最后更新:2026-05-20 / v1.90 管家 P1(章末元监督上线)*
