...
**最新里程碑**:`(待推)` — BUG-020 短回复判定提速:轮询 1s→0.3s,等待 30s→15s,短稳定 4s→1.5s,stop 按钮 SVG rect 兜底
**前几个里程碑**:
- `ab68a13` — BUG-019 完成判定加 stop 按钮信号 + 内容兜底
- `49b90e8` — DeepSeek textarea React 注入 + 邻近发送按钮策略
...
*最后更新:2026-05-16 第二十二批 / BUG-020 短回复判定提速 1 分多钟 → 几秒*
