...
**最新里程碑**:`(待推)` — **v2.03 🎯 模块化拆分 P4 — `ui/tabs/` 子包(8 个 Tab,3265 行)+ `core/default_skills.py` + `_profile_for_url` 迁移**。建 `ui/tabs/` 子目录,抽出 8 个 QWidget Tab 类:`ChapterEditor`(404 行)/ `SkillLibrary`(242 行)/ `GenerationControl`(338 行)/ `ProjectHomeTab`(307 行)/ `DialogMemory`(203 行)/ `CanonGuard`(213 行)/ `BookSplitterTab`(170 行)/ `CreationSettings`(1388 行 — P4 单文件最大),按类名命名独立文件。同时迁移 `DEFAULT_SKILLS`(60 行写作技能模板)→ `core/default_skills.py`;迁移 `_profile_for_url` 函数(6 行 URL→profile 匹配)→ `core/site_profiles.py`(自然归属,SITE_PROFILES 旁边),避免 Tab 文件循环 import 主程序。**主程序 19302→15982 行,瘦 3320 行**(累计 P1+P2+P3+P4:20740→15982,**共瘦 4758 行 / 22.9%**)。APP_VERSION v2.02→**v2.03**。**预先 AST 依赖扫描发现的 3 个"非 Tab 类外部符号"**:(1)`_profile_for_url`(顶层函数,6 处调用,迁 site_profiles.py)+ (2)`SELENIUM_AVAILABLE`(模块级 flag,GenerationControl 用)+ (3)`BOOK_SPLITTER_AVAILABLE` + `book_splitter`(模块级 flag + 模块,BookSplitterTab 用)。**关键设计决策:可用性 flag 各文件独立 try/except 重定义,避免循环 import**(`generation_control.py` 顶部自己写 `try: import selenium; SELENIUM_AVAILABLE = True except ImportError: SELENIUM_AVAILABLE = False`,主程序保留自己的 flag 不变;`book_splitter.py` Tab 同样模式)。**抽取方法**:一次性 AST 脚本(`/home/claude/extract_p4.py`),按行号从大到小删除避免偏移,每个新文件加定制 header(精确 import 集合)。**测试静态扫描适配**:5 个测试文件(test_bug068~072)用 `find('def xxx')` / `assertIn('#3a2a10')` 等静态字符串扫描 `novel_ai.py`,P4 后 ChapterEditor 外迁导致 8 个测试挂(`_set_pangu_meta_display` / `pangu_hook_label` / `color:#3a2a10` 等找不到)— **不是 P4 引入的代码 bug,是测试设计与拆分耦合需要适配**。修法:test_bug068 用 `_load_all_sources()` 函数读 novel_ai.py + ui/tabs + ui + core 全部 concat;test_bug069/070/071/072 的 `test_v194_button_color_intact` 直接改读 `ui/tabs/chapter_editor.py`(因为 `#3a2a10` 字符串只在那个文件)。**测试零退化**:424 passed(BUG-065~072 套 136 + workflow 31 + research 123 + lifespan 68 + lifespan_panel 66),与 P1/P2/P3 基线完全一致。**给下一代 Claude 的接力告示**:P1+P2+P3+P4 已收官,主程序剩 15982 行 / 1MB → 660KB(瘦近 1/3),三大头部:`MainWindow`(~9400 行,巨型)+ `CharacterLibrary`(3862 行)+ `BrowserWorker`(2480 行)。**下一步 P5**:推荐先拆 `CharacterLibrary`(3862 行 / 13 子表/子页,自包含程度高,有 11 个 mixin 方法可独立);再后 P6 拆 `BrowserWorker`(2480 行 / Selenium 逻辑全自包含)。`MainWindow` 留到最后或干脆不拆(它是装配中心,内部 11000+ 行方法很多互相调用,拆 mixin 风险高,收益相对低)。**P4 经验教训**:(1)静态字符串扫描的测试会随拆分而失效,要么扩源要么改路径 — 下个 P 阶段一定要先 grep 测试文件里的 `find/assertIn/read_text` 提前发现待修测试;(2)循环 import 不一定要靠工厂模式,简单 flag 各文件独立 try/except 也行(轻量,可读性高);(3)对每个外迁文件做 `python3 -c "import xxx"` 单独 import 验证非常有用,能在跑测试前提早暴露 import 错误。

**前一个里程碑**:`2d09a33` — v2.02 P3 ui/ 子包 + core/site_profiles.py(5 类 + DOM 档案外迁,瘦 720 行)。
**再前**:`cd4cc3f` — v2.01 P2 PROMPTS 字典外迁(瘦 583 行)。
**再再前**:`fd1b0eb` — v2.00 P1 core/ 子包(constants + stylesheet,瘦 145 行)。
**再再再前**:`8142921` — v1.98 记忆校准修订(纯文档,把过期的 8233 行 / 17 keys / 8 个 .py 等数字全对齐到 20732 行 / 29 keys / 18 个 .py 真实状态;补 ⓪⓪.5 GitHub Web UI 文件列表截断警告)。
