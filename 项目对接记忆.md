...
**最新里程碑**:`(待推)` — v1.92 🔒 BUG-066 章节锁定机制 MVP(借鉴竞品"由风写作 v1.1.7"中稿/终稿锁定思路,跟 v1.91 BUG-065 "做对的事不能丢"同源焦虑)。**起因**:用户上传竞品 v1.1.7 功能介绍文档让 Claude 筛"可借鉴功能"。Claude 筛掉营销话术后,识别出真正高价值的是「中稿锁定 + 区间审查 + 逐级清除」三件套 — 本质是同一套"已确认内容不能被后续审查/修复/重生成误改"的工作流。**本轮范围(MVP,克制做到能用即可)**:章节 dict 新增 `locked: bool` 字段(默认 False);加载兜底兼容旧存档(`setdefault("locked", False)`,两处 load 入口都补);章节列表显示 🔒 前缀 + tooltip;章节列表右键菜单加「🔒 锁定本章」「🔓 解锁本章」(用 `_on_chapter_list_context_menu` + `_toggle_chapter_lock`,**避让原有的 `toggle_lock` — 它是编辑器只读切换,不是章节级锁定**);拦截四个改动入口:`delete_chapter` / `rename_chapter` / `save_current_chapter` / `_on_chapter_clicked` 切走时写回。**本 MVP 不做(留下一轮)**:housekeeper 不感知 locked(留 v1.93+);编辑器只读联动(留 v1.93);区间审查 / 逐级清除(需 UI 大改,留 v1.94);AI 审查链 / dialogue_critic autofix 拦截(等区间审查一起做)。**顺手修**:`test_bug065_send_retry_degrade.py` 里硬编码的 `assertIn("v1.91", line)` 改成 `(major, minor) >= (1, 91)` 语义 — 不然每升一个版本都要改一次测试。**测试 15 条全过 + BUG-065 17 条 + housekeeper 19 条**(共 51 条),`py_compile` OK。本次仅触动 `novel_ai.py`(12 处改动)+ 新增 1 个测试文件 + 改 1 个旧测试。
**前几个里程碑**:
...
**最近更新**:2026-05-21 — v1.92 🔒 BUG-066 章节锁定机制 MVP 上线。借鉴竞品"由风写作 v1.1.7"中稿/终稿设计,跟 v1.91 BUG-065 "做对的事不能丢"同源焦虑。新增章节 dict locked 字段 + 加载兼容 + 右键菜单锁解 + 4 个改动入口拦截。新测试 15 条全过,合跑 51 条全过。本次仅触动 novel_ai.py(12 处)+ 1 新测试 + 1 旧测试小修(版本号断言改 ≥ 语义)。
...
*最后更新:2026-05-21 / v1.92 BUG-066 章节锁定机制 MVP(借鉴竞品 + 同源焦虑)*
