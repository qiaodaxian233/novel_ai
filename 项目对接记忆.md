...
**最新里程碑**:`(待推)` — **v2.04 🎯 模块化拆分 P5 — `ui/tabs/character_library.py`(`CharacterLibrary` 3860 行外迁,P5 单文件最大)**。把 13 子表/子页 / 80 方法的 `CharacterLibrary` 类整体抽到 `ui/tabs/character_library.py`(182 KB,P5~v2.04 阶段单文件最大,P4 阶段最大是 `creation_settings.py` 1388 行)。**主程序 15982→12125 行,瘦 3857 行**(累计 P1+P2+P3+P4+P5:20740→12125,**共瘦 8615 行 / 41.5%**)。APP_VERSION v2.03→**v2.04**。**预先 AST 依赖扫描确认极度自包含**:80 方法但**零项目内类依赖**(不引用 MainWindow / BrowserWorker / 任何其他 Tab 类),只依赖 PyQt5(11 个 widget)+ stdlib(`json` + `re`)+ 1 个外部 flag(`RELATION_GRAPH_AVAILABLE`)+ 1 个外部模块(`relation_graph`)。**关键决策:`relation_graph` flag 走 P4 同模式**(`character_library.py` 顶部自己 try/except import,主程序 try/except 保留不变,各文件独立)。**抽取方法**:一次性 AST 脚本(`/home/claude/extract_p5.py`),定位 L225-L4084,提取后写带 header 的新文件,主程序原段删除。**测试静态扫描适配**:23 个测试挂(test_bug067 全套 19 + test_bug069/070/071/072 的 v193 char fields 共 4),全因 `extract_method_source(NOVEL_AI_PATH, "CharacterLibrary", ...)` 和 `assertIn('"last_ch"', src)` 等查 CharacterLibrary 内容的扫描点。修法:test_bug067 顶部加 `CHARLIB_PATH = HERE/ui/tabs/character_library.py`,7 处用 `NOVEL_AI_PATH` 查 CharacterLibrary 内容的改成 `CHARLIB_PATH`,另 2 处查 `_toggle_chapter_lock` / `"locked": False` 的保留 `NOVEL_AI_PATH`(仍在主程序);test_bug069/070/071/072 的 v193 char fields 测试改读 `ui/tabs/character_library.py`。**测试零退化**:424 passed,完全对齐 P1~P4 基线。**给下一代 Claude 的接力告示**:P1~P5 已收官,主程序剩 **12125 行 / 480KB**(从 20740 行 / 1MB 瘦 41.5%),三大头部剩两个:`MainWindow`(~9400 行,装配中心,11000+ 行方法互相调用多)+ `BrowserWorker`(2480 行,Selenium 逻辑,自包含程度高)。**下一步 P6**:拆 `BrowserWorker` 到 `workers/browser_worker.py`(或者直接 `ui/workers/`),应该跟 P5 同样低风险(自包含、零项目内类依赖)。`MainWindow` 留到最后或干脆不拆(它是 entry point,11000 行方法很难独立切,收益不高,易引入 bug)。**P5 经验教训**:(1)P4 的"测试静态扫描挂"模式重现:这次也是 23 个挂,但因为有 P4 经验,定位 + 批量改测试只花了 5 分钟;(2)单文件 3860 行抽出后**没有再继续拆 mixin**,事实证明:**类内部就是个大文件就让它是大文件,只要外部边界清晰,内部 80 个方法的紧密耦合是 OK 的**;(3)182 KB 单文件比 5431 行 GitHub 网页预览限制还小,GitHub 应该能完整显示——可以验证一下;(4)P5 风险评估"低"被完全验证,P6 的 BrowserWorker 同质应该也是低风险。

**前一个里程碑**:`87eaf1d` — v2.03 P4 ui/tabs/ 子包 8 Tab + core/default_skills.py + _profile_for_url 迁移(瘦 3320 行)。
**再前**:`2d09a33` — v2.02 P3 ui/ 子包 + core/site_profiles.py(5 类 + DOM 档案外迁,瘦 720 行)。
**再再前**:`cd4cc3f` — v2.01 P2 PROMPTS 字典外迁(瘦 583 行)。
**再再再前**:`fd1b0eb` — v2.00 P1 core/ 子包(constants + stylesheet,瘦 145 行)。
**再再再再前**:`8142921` — v1.98 记忆校准修订(纯文档)。
