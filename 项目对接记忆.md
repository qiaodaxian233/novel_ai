...
**最新里程碑**:`(待推)` — v1.95 📏 BUG-069 字数判定三档(✓ / ⚠ / ✗,A→B→C→D 路线第一步)。**起因**:v1.94 实战日志里第 4 章 3088 字 / 目标 2300 = 1.34 倍,housekeeper 判 ✗ 让用户疑惑("章节质量没问题,只是字数偏多,为啥判失败?")。**修法**:字数评分从二档变三档 — `actual < 0.8x target` ✗(原行为,不够); `0.8x ≤ a/t ≤ 1.5x` ✓(合理,新增上限); `actual > 1.5x target` ⚠(超长,新增第三档,**仍算 ok=True 不扣健康度**,oneliner 显示 ⚠ 提醒)。**关键设计**:加新字段 `word_count_long: bool = False`(不动 `word_count_ok` 语义)— 既有 19 条 test_housekeeper 测试**一字未改全过**,完美兼容证据。**测试 19 条全过**:8 条三档判定 + 4 条 oneliner emoji 显示 + 2 条健康度评分(超长不扣分,不够仍扣)+ 1 条字段默认值 + 4 条 v1.91-v1.94 完整性。**合跑回归 101 条全过**(19 BUG-069 + 7 BUG-068 + 24 BUG-067 + 15 BUG-066 + 17 BUG-065 + 19 housekeeper)。本次仅触动 `housekeeper.py`(3 处:字段 + record_word_count + render_oneliner)+ `novel_ai.py` (1 处 APP_VERSION) + 新增 1 测试。**A→B→C→D 路线**:A 完成,接下来 B(单章模式"批量已结束"治本)→ C(区间审查 + 逐级清除)→ D(让 AI 用 last_ch)。
**前几个里程碑**:
- `ed7e3c9` — v1.94 BUG-068 下一章选项按钮对比度修复。setStyleSheet 漏 color → 米色背景上文字不可见。加 `color:#3a2a10`(WCAG AAA)+ hover `color:#000`。教训:setStyleSheet 漏字段 = 用默认值;UI bug 必须靠截图暴露。
- `ce23313` — v1.93 BUG-067 角色 last_ch 字段 + 同名不同姓检查。tbl_chars 列 8→9 + `_find_duplicate_names` staticmethod 纯函数。关键漏改修复 L6158 load 路径列数沉默 bug。完全不动 v1.92。
...
*最后更新:2026-05-21 / v1.95 BUG-069 字数判定三档(A→B→C→D 第一步,加字段不改语义,既有 19 测试一字未改全过)*
