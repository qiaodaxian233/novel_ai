...
> **当前版本 `v1.90`**(每次推送后请同步更新此处)。**忘改 = 反例,跟"先推代码后推记忆"同等严重**。
> 详见"五十五、版本号铁律"那一节。
...
*最后更新:2026-05-21 / v1.97 BUG-071 `_pending_task_target` 改字典治本(A→B→C→D B-2 收官,**B 完整**)。注:上一轮 Claude 曾完成 v1.97 改动但 push 前 rm 沙箱导致本地 commit 丢失 → 本轮重做,严守"清场必须在远端二次确认后"铁律。*
