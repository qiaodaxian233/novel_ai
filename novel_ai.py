...
def main():
    ...
    from PyQt5.QtGui import QPixmap, QFont as _QFont
    _splash_pix = None
    if os.path.exists(_icon_path):
        _splash_pix = QPixmap(_icon_path).scaled(
            480, 480, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    splash = None
    if _splash_pix and not _splash_pix.isNull():
        splash = QSplashScreen(_splash_pix)
        splash.setFont(_QFont("Microsoft YaHei", 11))
        splash.show()
        splash.showMessage("正在加载盘古写作引擎...",
            Qt.AlignBottom | Qt.AlignHCenter, QColor("#FFD700"))
        app.processEvents()
    ...
    try:
        ...
        if splash:
            splash.showMessage("加载完成!",
                Qt.AlignBottom | Qt.AlignHCenter, QColor("#00FF00"))
            app.processEvents()
