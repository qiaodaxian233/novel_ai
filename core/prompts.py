...
PROMPTS = {
    "creative_inspiration": (
        ...
        "- 不能有血腥暴力色情违规内容\n\n"
        "【输出格式】每个创意一行,严格按此格式:\n"
        "1. 【你想的具体卖点词】这本书的详细说明(50字内)\n"
        "2. 【另一个具体卖点词】yyy\n\n"
        "❗❗❗ 格式硬要求(违反会被弹窗骂):\n"
        "  - 【】里**必须**是你自己想的有冲击力的卖点词,例如:\n"
        "    ✓ 【厨子修仙】 ✓ 【系统种田】 ✓ 【废柴逆袭】 ✓ 【马甲女帝】\n"
        "  - **绝对不能**写成 【一句话卖点】、【卖点】、【标题】这种占位字面!\n"
        "  - 这五个【】里的字会显示在用户的选择按钮上,要有吸引力。\n\n"
        "请直接输出5个创意,不要其他内容。"
    ),
    ...
    "outline_full": (
        ...
        "【重要】如果设定中指定了男主/女主名字,必须使用指定的名字,不要自己取名。"
        "每个角色必须有独立的名字,绝对不能出现两个角色同名的情况。\n\n"
        "【输出格式要求-必须严格遵守】\n"
    ...
    "chapter": (
        ...
        "7. 【反重复铁律】绝对不能重复前面章节已经写过的场景——"
        "不能做同样的菜、不能说同样的话、不能发生同样的事件。"
        "每一章必须推进新的情节,不能原地踏步\n"
    ),
    ...
    "world_extract": (
        ...
        "  ],\n"
        '  "hero_state": {{"age": "本章末主角年龄", "realm": "本章末主角身份/境界/职位", "location": "本章末主角所在地", "faction": "本章末主角所属组织/家族/公司", "mood": "本章末主角心境(如:愤怒/决绝/平静)"}}\n'
        "}}\n\n"
        "提取规则:\n"
    ...
    "canon_audit": (
        ...
        "4. 场景重复(注意区分'延续'和'重复'):\n"
        "   ✅ 延续=上一章结尾的场景在本章开头继续发展(允许,不算违规)\n"
        "   ❌ 重复=前面章节已完整写过的场景,本章又从头来一遍"
        "(同样的菜从头做、同样的对话再说一次、同样的人再递一次名片)\n"
        "   只有❌重复才算违规,severity=high,"
        "desc中注明'场景重复:与第N章的XX场景雷同'\n"
        "5. 没问题就返回 OK\n\n"
        "请直接输出严格 JSON,不要任何前后缀、不要 markdown 代码块,格式:\n"
        '{{"violated": false, "items": []}}\n'
        "或者:\n"
        '{{"violated": true, "items": [{{"severity":"high","desc":"场景重复:与第1章的家宴红烧肉场景雷同,相同的菜品和流程"}}]}}'
    ),
    ...
    "critique_character": (
        ...
        "注意:角色嘴上说的与心里想的/手上做的不一致,属于正常潜台词,不算 OOC;"
        "只有行为违背核心动机或能力设定才扣分。\n"
        "及格线 = 7,8 分以上为佳。\n\n"
        "【角色档案】\n{characters}\n\n"
        "【新章节】\n{content}\n\n"
    ...
    "critique_mismatch": (
        "你是小说角色鲜活度诊断师。有魅力的角色,嘴上说的、心里想的、手上做的三层必有错位;"
        "全员心口如一就是纸片人。请检查下面这章的重要角色,1-10 打分。\n"
        "重点扣分项:\n"
        "① 心口如一:重要角色台词=心理=行动,全程零错位、零潜台词\n"
        "② 错位无因:错位了但看不出动机(面子/利益/恐惧/在乎)\n"
        "③ 直给拆穿:叙述直接解释\"他嘴上这么说其实心里…\",替读者点破\n"
        "④ 错位泛滥:路人和功能性角色也强行加内心戏\n"
        "及格线 = 7,8 分以上为佳。\n\n"
        "章节正文:\n{content}\n\n"
        "请直接输出严格 JSON,不要任何前后缀、不要 markdown 代码块,格式:\n"
        '{{"score":6,"reason":"主角全章心口如一零潜台词;第7段反派动机被叙述直接点破"}}'
    ),

    "critique_pov_lock": (
        "你是小说视角诊断师。一个场景内视角必须锁定一个人物,转场才能换;"
        "场景内乱切视角会让读者情感脱节。请检查下面这章,1-10 打分。\n"
        "重点扣分项:\n"
        "① 场景内跳视角:同一场景一会儿写甲的内心一会儿写乙的内心\n"
        "② 上帝视角乱入:非视角人物的心理活动被直接写出,而不是通过动作神态呈现\n"
        "③ 无标记转场:视角切换处没有场景/时间转换标记,读者跟丢\n"
        "及格线 = 7,8 分以上为佳。\n\n"
        "章节正文:\n{content}\n\n"
        "请直接输出严格 JSON,不要任何前后缀、不要 markdown 代码块,格式:\n"
        '{{"score":5,"reason":"第3段同一酒桌连切三人内心;第8段路人心理直写"}}'
    ),

    "critique_mru": (
        "你是小说代入感诊断师。读者靠\"刺激→感受→反应(先动作后语言)\"的顺序入戏,"
        "顺序一乱就出戏。请逐段检查下面这章的情绪节点,1-10 打分。\n"
        "重点扣分项:\n"
        "① 无源反应:读者还没看到刺激,先看到了人物的动作/台词/情绪\n"
        "② 顺序颠倒:先语言后动作、先反应后刺激(要素可缺省,顺序不可乱)\n"
        "③ 刺激模糊:只写\"气氛变了\"\"事情不对劲\",没有具体可感的刺激源\n"
        "④ 感受拖沓:情绪渲染超过一短句,或情绪强度与刺激不匹配\n"
        "及格线 = 7,8 分以上为佳。\n\n"
        "章节正文:\n{content}\n\n"
        "请直接输出严格 JSON,不要任何前后缀、不要 markdown 代码块,格式:\n"
        '{{"score":6,"reason":"第4段主角先拔刀后才写黑影出现,无源反应;第9段刺激源模糊"}}'
    ),
...
"novel_to_script": (
        "你是专业短剧分镜导演，精通 AI 视频生成(可灵/Sora/Runway)的提示词写法。\n"
        "把下面的小说章节改编为【AI 视频级分镜脚本】，可直接用于 AI 生图/视频生成。\n\n"
        "═══════════════════════════════════\n"
        "【第一步：场景定义 JSON（每个独立场景各写一个）】\n"
        "═══════════════════════════════════\n"
        "###场景N：[场景名称]\n"
        "```json\n"
        "{{\n"
        "  \"Style\": \"超写实仿真人风格\",\n"
        "  \"Asset_Description\": \"纯环境，无人物，描述场地外观与质感\",\n"
        "  \"Scene_Fixed_Environment\": \"固定空间细节：家具/道具位置，跨镜头必须一致\",\n"
        "  \"Scene_Fixed_Lighting\": \"固定光源方向/色温/阴影，全集保持不变\",\n"
        "  \"Scene_Fixed_Background\": \"固定背景元素，全集保持不变\"\n"
        "}}\n"
        "```\n\n"
        "═══════════════════════════════════\n"
        "【第二步：分镜信息表（Markdown 表格）】\n"
        "═══════════════════════════════════\n"
        "表头：|分镜|场景|出场场景|出场人物|道具|画面图片提示词|画面视频提示词|\n"
        "分隔行：|:---|:---|:---|:---|:---|:---|:---|\n\n"
        "填写规则：\n"
        "- 分镜：全集连续序号\n"
        "- 场景：对应第一步 JSON 场景名\n"
        "- 出场场景：该镜头环境细节（人物位置/时间/特殊状态）\n"
        "- 出场人物：参与角色，无人写【无（纯环境）】\n"
        "- 道具：画面关键道具\n"
        "- 画面图片提示词：风格+主体+镜头语言+光影+画质，用于 AI 生图\n"
        "- 画面视频提示词：【动态时间轴动作流】格式：\n"
        "  0~5秒：[精确到肌肉/角度/速度的动作描述]。\n"
        "  5~10秒：[动作描述]。\n"
        "  10~15秒：[衔接锚点]动作固定姿态描述。\n"
        "  BGM参考：[情绪+乐器描述]。\n"
        "  音效参考：[具体声音清单]。\n"
        "  衔接说明：[与下一分镜的连接方式，叠化/切换/拉远]\n\n"
        "═══════════════════════════════════\n"
        "【改编原则】\n"
        "═══════════════════════════════════\n"
        "- 开头 5 秒必须有视觉钩子（冲突/悬念/强烈反差）\n"
        "- 每个分镜 10~15 秒，每集 10~15 个分镜，全集 2~3 分钟\n"
        "- 台词外化：内心活动改成表情/动作/道具特写\n"
        "- 场景 JSON 固定元素在所有分镜中保持空间一致性\n"
        "- 末尾分镜留视觉悬念作为【衔接锚点】，供下集起始\n"
        "- 动作描述精确到毫米/秒数，直接可输入 AI 视频模型\n\n"
        "【小说原文（{word_count}字）】\n{content}\n\n"
        "严格按格式输出：先输出所有场景 JSON，再输出完整分镜表格。不要任何额外解释。"
    ),
